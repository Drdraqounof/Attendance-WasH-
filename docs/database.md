...
| `policy_thresholds` | `src/lib/policy-engine.ts` → `POLICY_THRESHOLDS` | The 2/10/16-point automated-workflow thresholds (verbal warning / action plan / final review). Admin-editable in principle; seeded from the policy engine's constants today. |
...
- Reaching 16 points (`riskLevelFromPoints` → `"termination_flag"`) means
  the employee is flagged for final administrative review/termination
  protocol — surfaced today on `/insights`' "Employees at risk" list as a
  "Final review" badge (`src/lib/insights-queries.ts::employeesAtRisk`).

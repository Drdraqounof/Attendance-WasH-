...
| `point_rules` | `src/lib/policy-engine.ts` → `ESCALATION_RULES` | The four real escalation tiers only — the old separate "positive points" + "deduction" catalog (`settings-mock.ts`) was retired; the model is deduction-only. `code`/`points`/`active` are populated for every row now. |
...
- **2026-09-06 update:** the legacy mock-driven pages (`/dashboard`,
  ...
  keeping its own 12-point copy, `people-mock.ts`'s demo data was
  retuned to a 16-point cap (Marcus Hale at 16 demonstrates "On PIP",
  Priya Nandakumar at 12 demonstrates "Required Manager Meeting"), and
